...
output: env
	@echo -e $(bold)$(primary)feed_processor_darklist_arn$(clear) = $(shell terraform -chdir=plans output feed_processor_darklist_arn)
	@echo -e $(bold)$(primary)feed_processor_darklist_role$(clear) = $(shell terraform -chdir=plans output feed_processor_darklist_role)
	@echo -e $(bold)$(primary)feed_processor_darklist_role_arn$(clear) = $(shell terraform -chdir=plans output feed_processor_darklist_role_arn)
	@echo -e $(bold)$(primary)feed_processor_darklist_policy_arn$(clear) = $(shell terraform -chdir=plans output feed_processor_darklist_policy_arn)
...
tail-aws-logs: ## Install using pipx install awscliv2
	awsv2 logs tail "/aws/lambda/$(shell sed -e 's/\(.*\)/\L\1/' <<< "${APP_ENV}")-feed-processor-darklist" --follow
