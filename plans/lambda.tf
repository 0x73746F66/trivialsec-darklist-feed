resource "aws_lambda_function" "feed_processor_darklist" {
  ...
  role             = aws_iam_role.feed_processor_darklist_role.arn
  handler          = "app.handler"
...
resource "aws_cloudwatch_event_rule" "feed_processor_darklist_schedule" {
  count               = var.app_env == "Prod" ? 1 : 0
  name                = "${lower(var.app_env)}_feed_processor_darklist_schedule"
...
resource "aws_cloudwatch_event_target" "schedule_lambda" {
  count     = var.app_env == "Prod" ? 1 : 0
  rule      = one(aws_cloudwatch_event_rule.feed_processor_darklist_schedule[*].name)
  target_id = "${lower(var.app_env)}_feed_processor_darklist"
  arn       = aws_lambda_function.feed_processor_darklist.arn
}

resource "aws_lambda_permission" "allow_events_bridge_to_run_lambda" {
  statement_id  = "${var.app_env}AllowExecutionFromCloudWatch"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.feed_processor_darklist.function_name
  principal     = "events.amazonaws.com"
}

resource "aws_cloudwatch_log_group" "darklist_logs" {
  skip_destroy      = var.app_env == "Prod"
  name              = "/aws/lambda/${aws_lambda_function.feed_processor_darklist.function_name}"
