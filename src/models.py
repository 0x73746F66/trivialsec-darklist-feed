...
class Darklist(BaseModel, DAL):
    ...
    def load(
        ...
        if ip_address:
            self.address_id = uuid5(internals.DARKLIST_NAMESPACE, str(ip_address))
            self.ip_address = str(ip_address)

        response = services.aws.get_dynamodb(table_name=services.aws.Tables.EWS_DARKLIST, item_key={'address_id': str(self.address_id)})
        if not response:
    ...
    def save(self) -> bool:
        return services.aws.put_dynamodb(table_name=services.aws.Tables.EWS_DARKLIST, item=self.dict())

    def delete(self) -> bool:
        return services.aws.delete_dynamodb(table_name=services.aws.Tables.EWS_DARKLIST, item_key={'address_id': str(self.address_id)})
